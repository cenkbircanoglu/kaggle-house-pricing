...
def get_best_model(model, X, Y, activation):
    ...
    history = model.fit(X, Y, batch_size=8, epochs=1,
                        verbose=2, validation_split=0.1, callbacks=[
            earlyStopping,
            saveBestModel,
            tensorboard
        ])
...
for activation in [ "tanh", "sigmoid",
    ...
    models.append(mlp6(tr_X.shape[1], activation))
    models.append(mlp7(tr_X.shape[1], activation))
...
for dnn_model, activation in models:
    ...
    print("CV, %s, %s, %s" % (dnn_model.name, activation, str(cv_score.mean())))
